...
events. `setDeviationBps` enforces `0 < deviationBps <= MAX_DEVIATION_BPS` (1,000 bps).
STRCMirrorModule keeps the deployed v1 `StrcPriceOracle`.

Oracle rotation remains callable while the vault is paused or the current wrapper
